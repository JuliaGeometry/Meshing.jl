using Meshing
using Test
using GeometryBasics
using ForwardDiff
using StaticArrays
using Statistics: mean
using LinearAlgebra: dot, norm
using GeometryBasics: mesh

algos = (MarchingCubes, MarchingTetrahedra, NaiveSurfaceNets)

function scalar_minimum(a::AbstractVector{T}) where T
    reduce((x, v)-> min.(x, v), a; init=T(typemax(eltype(T))))
end

function scalar_maximum(a::AbstractVector{T}) where T
    reduce((x, v)-> max.(x, v), a; init=T(typemin(eltype(T))))
end

@testset "meshing algorithms" begin
    for algo in (MarchingCubes, MarchingTetrahedra, NaiveSurfaceNets)
        @test algo(5) == algo{Float64}(5.0, 0.001, true, false)
        @test algo(0x00, 0x00) == algo{UInt8}(0x00, 0x00, true, false)
        @test algo{UInt16}(5, 0x00) == algo{UInt16}(0x0005, 0x0000, true, false)
        @test algo{Float32}(1) ==  algo{Float32}(1.0f0, 0.001f0, true, false)
        @test algo{Float32}(1.0, 0x00) == algo{Float32}(1.0f0, 0.0f0, true, false)
        @test algo{Float32}(iso=1, eps=0x00, insidepositive=true, reduceverts=false) ==
            algo{Float32}(1.0f0, 0.0f0, false, true)
    end
end

@testset "type helpers" begin
    dt = Meshing._determine_types
    @test dt(Point3{Float64}, TriangleFace{Int}) == (Point{3,Float64}, TriangleFace{Int64})
    @test dt(Point3f0, GLTriangleFace) == (Point3f0, GLTriangleFace)
    @test dt(nothing, nothing, Float16) == (Point3{Float16}, TriangleFace{Int64})
    @test dt(nothing, nothing, Float64, 4) == (Point{3,Float64}, QuadFace{Int64})
end

@testset "surface nets" begin
    sdf_sphere = SignedDistanceField(Rect(Vec(-1,-1,-1.),Vec(2,2,2.))) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end
    sdf_torus = SignedDistanceField(Rect(Vec(-2,-2,-2.),Vec(4,4,4.)), 0.05) do v
        (sqrt(v[1]^2+v[2]^2)-0.5)^2 + v[3]^2 - 0.25
    end

    snf_torus = mesh(Rect(Vec(-2,-2,-2.),Vec(4,4,4.)), (81,81,81), NaiveSurfaceNets()) do v
        (sqrt(v[1]^2+v[2]^2)-0.5)^2 + v[3]^2 - 0.25
    end

    snf_sphere = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)), (21,21,21), NaiveSurfaceNets()) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end

    # test convience constructors
    mesh(Rect(Vec3(-1.0),Vec3(2.0)), NaiveSurfaceNets()) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end
    mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)), NaiveSurfaceNets(), samples=(5,5,5)) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end

    sphere = mesh(sdf_sphere, NaiveSurfaceNets())
    torus = mesh(sdf_torus, NaiveSurfaceNets())
    @test length(coordinates(sphere)) == 1832
    @test length(coordinates(torus)) == 5532
    @test length(faces(sphere)) == 1830
    @test length(faces(torus)) == 5532

    @test length(coordinates(sphere)) == length(coordinates(snf_sphere))
    @test length(coordinates(torus)) == length(coordinates(snf_torus))
    @test length(faces(sphere)) == length(faces(snf_sphere))
    @test length(faces(torus)) == length(faces(snf_torus))
end


@testset "noisy spheres" begin
    # Produce a level set function that is a noisy version of the distance from
    # the origin (such that level sets are noisy spheres).
    #
    # The noise should exercise marching tetrahedra's ability to produce a water-
    # tight surface in all cases (unlike standard marching cubes).
    #
    N = 10
    sigma = 1.0
    distance = Float32[ sqrt(Float32(i*i+j*j+k*k)) for i = -N:N, j = -N:N, k = -N:N ]
    distance = distance + sigma*rand(2*N+1,2*N+1,2*N+1)

    # Extract an isosurface.
    #
    lambda = N-2*sigma # isovalue

    msh = mesh(distance, MarchingTetrahedra(lambda))

    s2 = SignedDistanceField(Rect(Vec(0,0,0.),Vec(1,1,1.))) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end

    msh = mesh(s2, MarchingTetrahedra())
    @test length(coordinates(msh)) == 973
    @test length(faces(msh)) == 1830
end

@testset "vertex interpolation" begin
    @test Meshing.vertex_interp(0, Vec(0,0,0), Vec(0,1,0), -1, 1) == Vec(0,0.5,0)
    @test Meshing.vertex_interp(-1, Vec(0,0,0), Vec(0,1,0), -1, 1) == Vec(0,0,0)
    @test Meshing.vertex_interp(1, Vec(0,0,0), Vec(0,1,0), -1, 1) == Vec(0,1,0)
end

@testset "marching cubes" begin

    algo = MarchingCubes()
    algo_pos = MarchingCubes(insidepositive=true)

    sdf = SignedDistanceField(Rect(Vec(-1,-1,-1.),Vec(2,2,2.))) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end

    mf = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)),algo, samples=(21,21,21)) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end

    mf_pos = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)),algo_pos, samples=(21,21,21)) do v
        -sqrt(sum(dot(v,v))) + 1 # sphere positive inside
    end

    mfrv = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)),MarchingCubes(reduceverts=false), samples=(21,21,21)) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end

    # convience constructors
    mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)), MarchingCubes()) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end
    mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)), MarchingCubes(), samples=(5,6,7)) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end

    @test length(coordinates(mfrv)) == 10968
    m = mesh(sdf,algo)
    m2 = mesh(sdf,algo)
    @test length(coordinates(m)) == 7320
    @test length(faces(m)) == 3656
    @test length(faces(mf)) == length(faces(mfrv))
    @test length(faces(mf)) == length(faces(mf_pos))
    @test m == m2
    @test length(coordinates(m)) == length(coordinates(mf))
    @test length(coordinates(mf)) == length(coordinates(mf_pos))
    @test length(faces(m)) == length(faces(mf))
end

@testset "marching tetrahedra" begin
    a1 = MarchingTetrahedra()
    a2 = MarchingTetrahedra(reduceverts=false)
    m1 = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)), a1, samples=(21,21,21)) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end
    m2 = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)), a2, samples=(21,21,21)) do v
        sqrt(sum(dot(v,v))) - 1 # sphere
    end
    @test length(coordinates(m1)) == 5582
    @test length(coordinates(m2)) == 33480
    @test length(faces(m1)) == 11160
    @test length(faces(m2)) == length(faces(m1))
end

@testset "sign flips" begin
    for algo_type in algos
        algo = algo_type()
        algo_pos = algo_type(insidepositive=true)
        mf = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)),algo, samples=(21,21,21)) do v
            sqrt(sum(dot(v,v))) - 1 # sphere
        end

        mf_pos = mesh(Rect(Vec(-1,-1,-1.),Vec(2,2,2.)),algo_pos, samples=(21,21,21)) do v
            -sqrt(sum(dot(v,v))) + 1 # sphere positive inside
        end
        @test coordinates(mf) == coordinates(mf_pos)
        @test faces(mf) == faces(mf_pos)
    end
end

@testset "function/array" begin

    sdf_torus = SignedDistanceField(Rect(Vec(-2,-2,-2.),Vec(4,4,4.)),0.05) do v
        (sqrt(v[1]^2+v[2]^2)-0.5)^2 + v[3]^2 - 0.25 # torus
    end

    fn_torus(v) = (sqrt(v[1]^2+v[2]^2)-0.5)^2 + v[3]^2 - 0.25 # torus

    for algo in algos
        @testset "$algo" begin
            sdf_m = mesh(sdf_torus, algo())
            fn_m = mesh(fn_torus, Rect(Vec(-2,-2,-2.),Vec(4,4,4.)), size(sdf_torus), algo())
            @test coordinates(sdf_m) == coordinates(fn_m)
            @test faces(sdf_m) == faces(fn_m)
        end
    end
end

@testset "respect origin" begin
    # verify that when we construct a mesh, that mesh:
    #   a) respects the origin of the SDF
    #   b) is correctly spaced so that symmetric functions create symmetric meshes
    f = x -> norm(x)
    bounds = Rect(Vec(-1, -1, -1), Vec(2, 2, 2))
    resolution = 0.1
    sdf = SignedDistanceField(f, bounds, resolution)

    for algorithm in (MarchingCubes(0.5),
                      MarchingTetrahedra(0.5),
                      MarchingCubes(iso=0.5,reduceverts=false),
                      MarchingTetrahedra(iso=0.5,reduceverts=false))
        @testset "$(typeof(algorithm))" begin
            sdfmesh = mesh(sdf, algorithm)
            # should be centered on the origin
            @test mean(coordinates(sdfmesh)) ≈ [0, 0, 0] atol=0.15*resolution
            # and should be symmetric about the origin
            @test scalar_maximum(coordinates(sdfmesh)) ≈ [0.5, 0.5, 0.5] atol=0.1
            @test scalar_minimum(coordinates(sdfmesh)) ≈ [-0.5, -0.5, -0.5] atol=0.1

            # Test functional Meshing
            fmesh = mesh(f, bounds, size(sdf), algorithm)
            # should be centered on the origin
            @test mean(coordinates(fmesh)) ≈ [0, 0, 0] atol=0.15*resolution
            # and should be symmetric about the origin
            @test scalar_maximum(coordinates(fmesh)) ≈ [0.5, 0.5, 0.5] atol=0.1
            @test scalar_minimum(coordinates(fmesh)) ≈ [-0.5, -0.5, -0.5] atol=0.1
        end
    end

    #TODO: SurfaceNets functional variant is bugged
    # Naive Surface Nets has no accuracy guarantee, and is a weighted sum
    # so a larger tolerance is needed for this one. In addition,
    # quad -> triangle conversion is not functioning correctly
    # see: https://github.com/JuliaGeometry/GeometryBasics.jl/issues/169
    sdfmesh = mesh(sdf, NaiveSurfaceNets(0.5))
    # should be centered on the origin
    @test mean(coordinates(sdfmesh)) ≈ [0, 0, 0] atol=0.15*resolution
    # and should be symmetric about the origin
    @test scalar_maximum(coordinates(sdfmesh)) ≈ [0.5, 0.5, 0.5] atol=0.2
    @test scalar_minimum(coordinates(sdfmesh)) ≈ [-0.5, -0.5, -0.5] atol=0.2
end

@testset "AbstractMeshingAlgorithm interface" begin
    f = x -> norm(x) - 0.5
    bounds = Rect(Vec(-1, -1, -1), Vec(2, 2, 2))
    resolution = 0.1
    sdf = SignedDistanceField(f, bounds, resolution)

    for algo in algos
        @testset "$algo" begin
            @test_nowarn mesh(sdf, algo())
            mesh(sdf, algo())
            @test_nowarn mesh(sdf.data, algo(0.5))
            mesh(sdf.data, algo(0.5))
        end
    end
end

@testset "mixed types" begin
    # https://github.com/JuliaGeometry/Meshing.jl/issues/9
    samples = randn(10, 10, 10)
    m1 = mesh(SignedDistanceField(
        Rect(Vec(0,0,0), Vec(1, 1, 1)),
        samples), MarchingTetrahedra())
    m2 = mesh(SignedDistanceField(
        Rect(Vec(0,0,0), Vec(1, 1, 1)),
        Float32.(samples)), MarchingTetrahedra())
    @test all(isapprox.(coordinates(m1), coordinates(m2), rtol=1e-6))
    @test all(faces(m1) == faces(m2))

    @testset "forward diff" begin
        # Demonstrate that we can even take derivatives through the meshing algorithm
        f = x -> norm(x)
        bounds = Rect(Vec(-1, -1, -1), Vec(2, 2, 2))
        resolution = 0.1
        sdf = SignedDistanceField(f, bounds, resolution)
        for algo in (MarchingCubes, MarchingTetrahedra)
            @testset "$algo" begin
                let
                    function surface_distance_from_origin(isovalue)
                        # Compute the mean distance of each vertex in the isosurface
                        # mesh from the origin. This should return a value equal to the
                        # isovalue and should have a derivative of 1.0 w.r.t. the
                        # isovalue. This function is just meant to serve as an example
                        # of some quantity you might want to differentiate in a mesh,
                        # and has the benefit for testing of having a trivial expected
                        # solution.
                        m = mesh(sdf, algo(isovalue))
                        mean(norm.(coordinates(m)))
                    end

                    isoval = 0.85
                    @test surface_distance_from_origin(isoval) ≈ isoval atol=1e-2
                    @test ForwardDiff.derivative(surface_distance_from_origin, isoval) ≈ 1 atol=1e-2
                end
            end
        end
    end

    @testset "type stability" begin
        # verify that we don't lose type stability just by mixing arguments
        # of different types
        data = randn(5, 5, 5)
        iso = 0.2
        eps = 1e-4
        for algo_ty in (MarchingTetrahedra, MarchingCubes,NaiveSurfaceNets)
            algo1 = algo_ty(iso, eps)
            algo2 = algo_ty(Float64(iso), Float16(eps))
            algo3 = algo_ty(Float32(iso), Float64(eps))
            @inferred(isosurface(data, algo1, Point{3, Float16}, TriangleFace{UInt32}))
            @inferred(isosurface(Float32.(data), algo2, Point{3, Float32}, TriangleFace{Int16}))
            @inferred(isosurface(Float64.(data), algo3, Point{3, Float64}, TriangleFace{UInt}))
        end
    end
    @testset "Float16" begin
        sdf_torus = SignedDistanceField(Rect(Vec{3,Float16}(-2,-2,-2.),
                                                        Vec{3,Float16}(4,4,4.)),
                                        0.1, Float16) do v
            (sqrt(v[1]^2+v[2]^2)-0.5)^2 + v[3]^2 - 0.25
        end

        m1 = mesh(sdf_torus, NaiveSurfaceNets(zero(Float16)), facetype=QuadFace{Int}, pointtype=Point3{Float16})
        @test (eltype(coordinates(m1)), eltype(faces(m1))) == (Point3{Float16}, QuadFace{Int})

        m1 = mesh(sdf_torus, MarchingTetrahedra(zero(Float16)), facetype=TriangleFace{Int}, pointtype=Point3{Float16})
        @test (eltype(coordinates(m1)), eltype(faces(m1))) == (Point3{Float16}, TriangleFace{Int})

        m1 = mesh(sdf_torus, MarchingCubes(zero(Float16)), facetype=TriangleFace{Int}, pointtype=Point3{Float16})
        @test (eltype(coordinates(m1)), eltype(faces(m1))) == (Point3{Float16}, TriangleFace{Int})

    end
    @testset "Integers" begin
        A = rand(Int, 10,10,10)
        for algo in algos
            @testset "$algo" begin
                p, t = isosurface(A, algo())
                @test typeof(p) <: Vector{SVector{3,Float64}}
                @test typeof(t) <: Vector{ SVector{ Meshing.default_face_length(algo()), Int } }
            end
        end
    end
    @testset "Defaults" begin
        A = rand(10,10,10)
        @test isosurface(A) == isosurface(A, MarchingCubes())
    end
end

@testset "GeometryBasics API" begin
    @testset "vararg passing" begin
        A = rand(20,20,20)
        m = mesh(A,MarchingTetrahedra(1.0),origin=Point(Float32(0),Float32(0),Float32(0)),widths=Point(Float32(1),Float32(1),Float32(1)))
        @test m isa Mesh
    end
end
