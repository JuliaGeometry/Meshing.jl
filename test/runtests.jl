...
const norm_sdf =[norm((x,y,z)) for x in -1:0.01:1, y in -1:0.01:1, z in -1:0.01:1]
const sphere_sdf =[sphere_function((x,y,z)) for x in -1:0.01:1, y in -1:0.01:1, z in -1:0.01:1]
        ...
        @testset "$algo" begin
            let
                function surface_distance_from_origin(isovalue)
                    ...
                    # solution.
                    rng = -2:0.01:2
                    sdf = [norm((x,y,z)) for x in rng, y in rng, z in rng]
                    points, _ = isosurface(sdf, algo(iso=isovalue), rng, rng, rng)
                    mean(norm.(points))
                end
...
@testset "respect origin" begin
    ...
    for algorithm in (MarchingCubes(iso=0.5),
        ...
        @testset "$(typeof(algorithm))" begin
            ...
            points, faces = isosurface(norm_sdf, algorithm)

            # should be centered on the origin
            @test mean(collect.(points)) ≈ [0, 0, 0] atol=0.015
